...
class IcuConan(ConanFile):
    ...
    def build_msys(self):
        ...
        for p in pathList:
            if p != "C:\\Program Files\\Git\\usr\\bin" and p != "C:\\Program Files\\Git\\cmd":
        ...
        #with tools.environment_append(env_build.vars):
        self.run("{vccmd} && bash -lc ^'cd {builddir} ^&^& {config_cmd}^'".format(vccmd=self.cfg['vcvars_command'],
        ...
        self.run("{vccmd} && bash -lc ^'cd {builddir} ^&^& make {silent} -j {cpus_var}^'".format(vccmd=self.cfg['vcvars_command'],
                                                                                              builddir=self.cfg['build_dir'],
                                                                                              silent=self.cfg['silent'],
                                                                                              cpus_var=tools.cpu_count()))
        if self.options.with_unit_tests:
            self.run("{vccmd} && bash -lc ^'cd {builddir} ^&^& make {silent} check^'".format(vccmd=self.cfg['vcvars_command'],
                                                                                          builddir=self.cfg['build_dir'],
                                                                                          silent=self.cfg['silent']))

        self.run("{vccmd} && bash -lc ^'cd {builddir} ^&^& make {silent} install^'".format(vccmd=self.cfg['vcvars_command'],
                                                                                         builddir=self.cfg['build_dir'],
                                                                                         silent=self.cfg['silent']))
